...
local framework, scrollHandler, fireSignal do
...
function RollAccuracy()
    local ACCURACY_NAMES = {'Sick', 'Good', 'Ok'}
    local Percentages = {
    ...
    for index, arrow in next, framework.UI.ActiveSections do
        if arrow.Side == framework.UI.CurrentSide then
            ...
            if position and (position == note_Position) then
                if (arrow.Data.Time - note_DataTime) < 0.25 then
					return tonumber(arrow.Data.Time - note_DataTime)/4
                end
            end
        end
    end
                ...
                if noteTime > Chances[RollAccuracy()] then
                    task.spawn(function()
                        ...
                        if arrow.Data.Length > 0 then
                            if arrow.Data.Length <= 0.15 then
                                task.wait(0.15)
                            else
                                task.wait(arrow.Data.Length + 0.05)
                            end
                        else
